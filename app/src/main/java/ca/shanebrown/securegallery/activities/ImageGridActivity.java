...
import ca.shanebrown.securegallery.util.ImageGridRecyclerAdapter;
import ca.shanebrown.securegallery.util.ImageHandler;
import ca.shanebrown.securegallery.util.ImagePathBundle;
import ca.shanebrown.securegallery.util.NoMarginDecoration;
...
public class ImageGridActivity extends AppCompatActivity implements ImageGridRecyclerAdapter.OnImageClickListener {
    ...
    private final SelectionModeCallback selectionModeCallback = new SelectionModeCallback();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(sent_uris != null && !sent_uris.isEmpty()){
            ...
            Log.i("SecureGallery", "Images recieved from another app");
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode == 100 && resultCode == RESULT_OK){
            ...
            if(data != null && data.getBooleanExtra("changed", false)) {
                Log.i("SecureGallery", "ImageViewActivity item updated, refreshing recycler");
    ...
    /**
     * Set "No Images" text when there's no images in the gallery
     * @param noImages Text visible when true, invisible when false
     */
    ...
    /**
     * Handles toggling selection when user presses on an image tile
     * @param position index of pressed image in adapter dataset
     */
    ...
    /**
     * Re-fetches images from secure store and updates recycler
     */
    ...
    /**
     * Handles user action of deleting selected images
     */
    private void deleteSelectedUserAction(){
        ...
        builder.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                if(deleteSelectedImages(items)){
                    Toast.makeText(ImageGridActivity.this, "Images deleted", Toast.LENGTH_SHORT).show();
                }else{
                    Toast.makeText(ImageGridActivity.this, "Error: One or more images were unsuccessfully deleted", Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    /**
     * Deletes selected images from secure store
     * @param items Selection items to delete
    ...
    /**
     * Handles the key press for adding images to the app from a content provider
     */
    ...
    /**
     * Handles user interaction for importing of images shared from another app
     * @param sent_uris URIs of sent images
    ...
    /**
     * Handles the importing of images shared from another app
     * @param sent_uris URIs of sent images
     * @param key Decryption phrase
     */
    private void handleSentItems(final ArrayList<Uri> sent_uris, final String key){
        ...
        // Refresh recycler with newly imported images
        try {
            images = ImageHandler.getAllSecureImageBitmaps(key, salt);
    ...
    /**
     * Handles user action of saving decrypted images based on selection
     */
    private void saveDecryptedImageSelectedUserAction(){
        ...
        if(success) {
            ...
            Toast.makeText(ImageGridActivity.this, "An error occurred when saving some or all images", Toast.LENGTH_SHORT).show();
        }
    }
